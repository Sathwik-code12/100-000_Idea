...
//   console.log("Fetched heros:", heros);
//   return (
...
export default function MainHero() {
  ...
  const heros = herosData?.hero || [];
  const activeHero = heros.find((h: any) => h.isActive); // pick the first active hero

  if (!activeHero) {
    return null; // or a fallback UI if no active hero
  }

  const { title, subtitle, cta, backgroundColor } = activeHero;
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
        ...
        <div className="flex justify-center">
          <Link href={cta.link}>
            <Button
              className="w-full sm:w-auto text-white px-6 lg:px-8 py-3 lg:py-4 rounded-lg font-bold text-base lg:text-lg shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-105"
              style={{ backgroundColor: cta.backgroundColor || "#000000" }}
            >
              {cta.label}
            </Button>
          </Link>
